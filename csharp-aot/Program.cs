...
static Node BottomUpTree(int depth) =>
    depth <= 0 ? new Node() : new Node(BottomUpTree(depth - 1), BottomUpTree(depth - 1));

class Node(Node? left = null, Node? right = null)
{
    public int ItemCheck() => left == null ? 1 : 1 + left.ItemCheck() + right!.ItemCheck();
}
